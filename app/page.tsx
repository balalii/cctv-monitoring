...
type ViewLevel = 'provinsi' | 'kabupaten';

// ... (semua 'variants' animasi tetap sama, tidak perlu diubah)
const pageTransitionVariants = {
  initial: { opacity: 0, scale: 0.97, filter: 'blur(4px)' },
  animate: { zIndex: 1, opacity: 1, scale: 1, filter: 'blur(0px)', transition: { duration: 0.5, ease: [0.25, 1, 0.5, 1] } },
  exit: { zIndex: 0, opacity: 0, scale: 1.03, filter: 'blur(4px)', transition: { duration: 0.3, ease: [0.5, 0, 0.75, 0] } },
};
const cardVariants = {
  hidden: { opacity: 0, y: 70, scale: 0.9, rotateX: 10 },
  visible: (index: number) => ({ opacity: 1, y: 0, scale: 1, rotateX: 0, transition: { delay: 0.6 + index * 0.1, duration: 0.8, ease: [0.16, 1, 0.3, 1], type: 'spring', stiffness: 100, damping: 15 } }),
  hover: { scale: 1.03, y: -8, rotateX: -2, boxShadow: '0 20px 40px rgba(0,0,0,0.1)', transition: { duration: 0.3, ease: [0.16, 1, 0.3, 1] } },
  tap: { scale: 0.98, transition: { duration: 0.1 } },
};
const statsVariants = {
  hidden: { scale: 0, opacity: 0, rotateY: 90 },
  visible: (index: number) => ({ scale: 1, opacity: 1, rotateY: 0, transition: { delay: index * 0.15, duration: 0.8, ease: [0.16, 1, 0.3, 1], type: 'spring', stiffness: 120, damping: 12 } }),
};
const headerVariants = {
  hidden: { opacity: 0, y: -60, scale: 0.95 },
  visible: { opacity: 1, y: 0, scale: 1, transition: { duration: 0.8, ease: [0.16, 1, 0.3, 1], staggerChildren: 0.1, delayChildren: 0.2 } },
};
const searchVariants = {
  hidden: { opacity: 0, y: 30, scale: 0.95 },
  visible: { opacity: 1, y: 0, scale: 1, transition: { duration: 0.7, ease: [0.16, 1, 0.3, 1], delay: 0.4 } },
};
const breadcrumbVariants = {
  hidden: { opacity: 0, x: -20 },
  visible: (index: number) => ({ opacity: 1, x: 0, transition: { delay: index * 0.1, duration: 0.5, ease: [0.16, 1, 0.3, 1] } }),
};
...
export default function CCTVMonitoringApp() {
  ...
  const [selectedKabupaten, setSelectedKabupaten] = useState<Kabupaten | null>(null); // State ini tetap untuk masa depan (view lokasi)
  ...
  const processedData = useMemo(() => {
    return indonesiaData.map((provinsi) => {
      ...
      return { ...provinsi, totalCCTV, onlineCount, alertCount, kabupatenCount };
    });
  }, []);

  // Efek untuk sinkronisasi state dari URL
  useEffect(() => {
    const provinsiId = searchParams.get('provinsi');

    if (provinsiId) {
      const provinsiFromUrl = processedData.find((p) => p.id.toString() === provinsiId);
      if (provinsiFromUrl) {
        setSelectedProvinsi(provinsiFromUrl);
      ...
      // Jika tidak ada parameter, pastikan tampilan kembali ke daftar provinsi
      setCurrentView('provinsi');
      setSelectedProvinsi(null);
      setSelectedKabupaten(null);
    }
  }, [searchParams, processedData, router]);

  // Handler klik kini mengubah URL
  const handleProvinsiClick = (provinsi: Provinsi) => {
    // Menambahkan { scroll: false } agar halaman tidak loncat ke atas
    router.push(`?provinsi=${provinsi.id}`, { scroll: false });
  };

  // Handler kembali kini mengubah URL ke root
  ...
  const getTotalStats = () => {
    if (currentView === 'provinsi') {
      ...
      return { total: selectedProvinsi.totalCCTV || 0, online: selectedProvinsi.onlineCount || 0, alerts: selectedProvinsi.alertCount || 0 };
    }
        ...
        <motion.div key={currentView} variants={pageTransitionVariants as any} initial="initial" animate="animate" exit="exit" className="min-h-screen">
          ...
          <motion.div className="bg-black sticky top-0 z-10 backdrop-blur-sm" variants={headerVariants as any} initial="hidden" animate="visible">
            <div className="px-4 py-4 pb-7">
              <motion.div className="flex items-center justify-between mb-4" variants={{ visible: { transition: { staggerChildren: 0.1 } } }}>
                <div className="flex items-center gap-3">
                  {currentView !== 'provinsi' && (
                    <motion.div initial={{ opacity: 0, x: -30, rotate: -90 }} animate={{ opacity: 1, x: 0, rotate: 0 }} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1], type: 'spring', stiffness: 100 }}>
                  ...
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.2, ease: [0.16, 1, 0.3, 1] }}>
              ...
              {currentView !== 'provinsi' && (
                <motion.div className="flex items-center gap-2 text-sm text-gray-200 mb-4" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3, ease: [0.16, 1, 0.3, 1] }}>
                  <motion.span
                    ...
                    whileTap={{ scale: 0.95 }}
                    onClick={handleBackClick} // Menggunakan handleBackClick yang sudah ada
                  ...
                  {selectedProvinsi && (
                    <>
                      ...
                      <motion.div
                        className={'text-gray-300'} // Tidak perlu lagi ada logic hover yang rumit
              ...
              {/* Stats Cards ... (tidak ada perubahan di sini) */}
          ...
          <div className="px-4 pt-1 pb-8 bg-gray-100">
            <motion.div className={`space-y-4 pb-5 min-h-[60vh]`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.4 }}>
              {/* Search Bar */}
              <motion.div className="relative" variants={searchVariants as any} initial="hidden" animate="visible">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4 z-10" />
                <Input
                  placeholder={`Cari ${currentView === 'provinsi' ? 'provinsi' : 'kabupaten/kota'}...`}
                  value={searchQuery}
                ...
                (filteredData as (Provinsi & { totalCCTV: number; onlineCount: number; alertCount: number; kabupatenCount: number })[]).map((provinsi, index) => (
                  <motion.div key={provinsi.id} variants={cardVariants as any} initial="hidden" animate="visible" whileHover="hover" whileTap="tap" custom={index} style={{ perspective: 1000 }}>
                    <Card className="hover:shadow-xl transition-all duration-500 cursor-pointer backdrop-blur-sm border-gray-200 hover:border-blue-300" onClick={() => handleProvinsiClick(provinsi)}>
                      {/* ... isi card provinsi sama seperti sebelumnya ... */}
                        ...
                        <div className="mt-3">
                          ...
                          <div className="w-full bg-slate-200 rounded-full h-2 overflow-hidden shadow-inner">
                            <motion.div
                              ...
                              animate={{ width: `${provinsi.totalCCTV && provinsi.totalCCTV > 0 ? (provinsi.onlineCount / provinsi.totalCCTV) * 100 : 0}%`, opacity: 1 }}
                              transition={{ duration: 1.2, delay: index * 0.15 + 0.8, ease: [0.16, 1, 0.3, 1] }}
                            />
                          </div>
                        </div>
                      </CardContent>
                ...
                (filteredData as Kabupaten[]).map((kabupaten, index) => (
                  <motion.div key={kabupaten.id} variants={cardVariants as any} initial="hidden" animate="visible" whileHover="hover" whileTap="tap" custom={index} style={{ perspective: 1000 }}>
                    <Card className="hover:shadow-xl transition-all duration-500 backdrop-blur-sm border-gray-200 hover:border-blue-300">
                      <Link href={kabupaten.url} className="block cursor-pointer" target="_blank" rel="noopener noreferrer">
                        {/* ... isi card kabupaten sama seperti sebelumnya ... */}
                        <CardHeader className="pb-2">
                          <div className="flex items-center justify-between">
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-1">
              ...
              {/* ... Tampilan 'Tidak ada data' sama seperti sebelumnya ... */}
              {filteredData.length === 0 && !searchQuery && (
                <motion.div className="text-center py-12" initial={{ opacity: 0, scale: 0.8, y: 50 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}>
